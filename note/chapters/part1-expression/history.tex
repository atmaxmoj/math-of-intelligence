...
\begin{itemize}
    ...
    \item See logic not as ``being rational'' but as the study of implication
    \item Trace the journey from Aristotle to the modern landscape
\end{itemize}
\end{goals}
...
Formal logic is not about being smart, or thinking clearly, or avoiding fallacies in everyday reasoning. It is something much more specific, and in a way, much stranger.

\begin{keyinsight}
Logic is the study of \textbf{implication}---the ``if... then...'' structure.

When can we say that one thing \emph{necessarily follows} from another? What does ``follows from'' even mean? These are the questions logic tries to answer, with mathematical precision.
\end{keyinsight}
...
\begin{history}[The Laws of Thought, 1854]
...
\begin{history}[Russell's Paradox]
...
\begin{keyinsight}
Logic studies \textbf{form} and \textbf{structure-preserving} reasoning.

\begin{itemize}
    \item \textbf{Form}: Logic abstracts away content, keeping only structure.
    \item \textbf{Structure-preserving}: Valid inference preserves truth. Consistent systems preserve non-contradiction. Computable functions preserve the finitary nature of calculation.
\end{itemize}
\end{keyinsight}
...
A question lingers: \textbf{can we study ``structure-preserving'' itself as a mathematical object?}

The four pillars use set theory as their common language. But sets are about \emph{membership} (what elements belong to what collections). Is there a language that directly talks about \emph{structure} and \emph{structure-preserving maps}?
...
\begin{summary}
\begin{itemize}
    \item Logic is not ``being rational''---it is the study of implication and valid inference
    \item It began with Aristotle's syllogism: form can be studied independently of content
    \item Frege created modern logic with quantifiers and predicates
    \item The foundational crisis led to four pillars: proof theory, model theory, computability, set theory
    \item All share a core insight: logic studies form and structure-preserving reasoning
    \item This is why diverse fields (philosophy, linguistics, computer science) all need logic
\end{itemize}
\end{summary}
