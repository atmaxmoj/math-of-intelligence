...
\begin{center}
\begin{tabular}{lll}
\textbf{Modus Ponens} & From $P$ and $P \to Q$, infer $Q$ & ``If it rains, the ground is wet. It rains. So the ground is wet.'' \\[0.5em]
\textbf{Modus Tollens} & From $\neg Q$ and $P \to Q$, infer $\neg P$ & ``If it rains, the ground is wet. The ground is dry. So it didn't rain.'' \\[0.5em]
\textbf{Hypothetical Syllogism} & From $P \to Q$ and $Q \to R$, infer $P \to R$ & Chaining conditionals \\[0.5em]
\textbf{Disjunctive Syllogism} & From $P \lor Q$ and $\neg P$, infer $Q$ & ``A or B. Not A. So B.''
\end{tabular}
\end{center}
...
\[
\forall x \, ((\mathrm{BlueGiraffe}(x) \land \mathrm{InThisRoom}(x)) \to \mathrm{TallerThanMe}(x))
\]
...
\textbf{Why it ``works'':} Premise 1 says ``no God $\Rightarrow$ the conditional is false.'' The contrapositive is ``conditional is true $\Rightarrow$ God exists.'' But since I don't pray, the conditional $P \to A$ is vacuously true! So by modus ponens, God exists.
\end{quote}
...
\begin{quote}
\textbf{Premise 1 (formal):} $\neg G \to \neg \Box(P \to A)$\\
\textbf{Premise 1 (plain):} ``If God doesn't exist, then it's \emph{not} necessarily true that praying leads to being answered.''\\[0.5em]
\textbf{Premise 2 (formal):} $\neg P$\\
\textbf{Premise 2 (plain):} ``I don't pray (in the actual world).''
\end{quote}

Now can we derive $G$ (God exists)?

\begin{enumerate}
    \item From $\neg P$, we get $P \to A$ in the actual world. \hfill (Vacuous truth, as before)

    \textit{Plain: ``Since I don't pray, `if I pray then I'm answered' is trivially true---here, now.''}

    \item But can we derive $\Box(P \to A)$? \hfill \textbf{No!}

    \textit{Plain: ``Does `I don't pray here' imply `in ALL situations, praying leads to answers'? Of course not!''}

    \item Without $\Box(P \to A)$, we cannot use the contrapositive of Premise 1.

    \textit{Plain: ``We can't say `necessarily praying works' just because we don't pray. So the trick that proved God exists no longer works.''}
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Morphisms} (arrows): maps under which structure is invariant
