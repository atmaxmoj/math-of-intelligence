...
\begin{question}[title={Q1: Completeness $\Rightarrow$ Decidability?}]
``If a proof exists (completeness), why can't I just search and find it? Doesn't that give me an algorithm (decidability)?''
\end{question}

\begin{question}[title={Q2: Decidability $\Rightarrow$ Completeness?}]
``If I have an algorithm that decides everything (decidability), doesn't that cover all truths (completeness)?''
\end{question}

Both questions have subtle answers.

\begin{answer}[title={A1: Completeness gives Semi-Decidability, not Decidability}]
Yes, you can search for proofs. Enumerate all possible proofs: proof 1, proof 2, proof 3, ... If $\varphi$ is valid, you'll eventually find a proof. But here's the catch: \emph{what if $\varphi$ is invalid?} Then no proof exists. You'll search forever---proof 1 (no), proof 2 (no), proof 3 (no)---never knowing whether to keep looking or give up.
...
\begin{answer}[title={A2: Decidability is about the Logic, Completeness is about a Proof System}]
You're right that if an algorithm says ``valid,'' you \emph{know} it's valid. But completeness asks a different question: can a \emph{specific proof system} derive all valid formulas?
...
A logic can be decidable while a particular proof system for it is incomplete.
\end{answer}
...
\begin{intuition}
Notice something: modus ponens is not just a ``rule''---it's an \textbf{operation}. Given inputs $A$ and $A \to B$, it produces output $B$. The proof system \emph{is} an algorithm, defined by its rules.

When we say ``use the decision algorithm as the proof system,'' we're really asking: which operations should count as valid proof steps? Truth tables? Modus ponens? Something else?

This connection---\textbf{proofs as computations, rules as operations}---runs deep. We'll return to it in proof theory, where the Curry-Howard correspondence reveals that proofs literally \emph{are} programs.
\end{intuition}

\textbf{Why not use the algorithm as the proof system?} You absolutely can! Define ``proof of $\varphi$'' to mean ``the truth table has all T's.'' Then every valid formula has a proof, and your system is complete.

This is a key insight: \textbf{if a logic is decidable, you can always construct a complete proof system}---just define ``proof'' as ``algorithm says valid.'' So:
\[ \text{Decidable} \Longrightarrow \text{Can have a complete proof system} \]

The modus-ponens-only example shows a \emph{different} proof system that is incomplete. The point is not that you \emph{must} use that system, but that completeness is a property of a \emph{particular} proof system, while decidability is a property of the \emph{logic itself}. The same logic can have complete and incomplete proof systems.

\begin{question}[title={Does completeness imply decidability?}]
If every valid formula has a proof (completeness), can't I just search through all proofs until I find one? Doesn't that decide validity?
\end{question}

\begin{answer}[title={No: Completeness gives Semi-Decidability, not Decidability}]
...
Here's what goes wrong. You want to know if $\varphi$ is valid:
\begin{enumerate}
    \item You enumerate all possible proofs: proof 1, proof 2, proof 3, ...
    \item If $\varphi$ is valid, by completeness, a proof exists. Eventually you find it. \textbf{Success!}
    \item But if $\varphi$ is \emph{invalid}, no proof exists. You search forever: proof 1 (no), proof 2 (no), ... \textbf{You never know when to stop.}
\end{enumerate}

Completeness guarantees the proof is out there \emph{if $\varphi$ is valid}. But it doesn't help when $\varphi$ is invalid---you can't distinguish ``haven't found it yet'' from ``doesn't exist.''

This is \textbf{semi-decidability}: you can confirm validity (by finding a proof) but cannot refute it (you'd search forever).
\end{answer}

\begin{question}[title={Does decidability imply completeness?}]
If I have an algorithm that decides validity, can't I just use ``algorithm says valid'' as the definition of ``has a proof''? Then every valid formula has a proof!
\end{question}

\begin{answer}[title={Yes, but Completeness is about a Specific Proof System}]
You're right! If the logic is decidable, you \emph{can} construct a complete proof system this way.

But completeness is a property of a \emph{particular} proof system, not the logic itself. The same decidable logic can have:
\begin{itemize}
    \item \textbf{System A} (algorithm-as-proof): Complete
    \item \textbf{System B} (modus ponens only): Incomplete
\end{itemize}

When we ask ``is this proof system complete?'' we're asking about \emph{that specific system}, not whether \emph{some} complete system exists.

The real lesson: decidable logic $\Rightarrow$ complete proof system \emph{exists}. But a given proof system might still be incomplete.
\end{answer}

The precise relationships:
\begin{align*}
\text{Decidable} &\Longrightarrow \text{Can construct a complete proof system} \\
\text{Complete proof system} &\Longrightarrow \text{Semi-decidable} \\
\text{Complete proof system} &\not\Longrightarrow \text{Decidable}
\end{align*}
