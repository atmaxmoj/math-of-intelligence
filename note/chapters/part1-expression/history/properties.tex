...
But notice: the moment you switch from ``modus ponens only'' to ``truth tables,'' you've changed proof systems. You're no longer discussing the original system---you've replaced it with a different one.

\begin{intuition}
This reveals something important: \textbf{choosing a proof system is choosing an algorithm}. Modus ponens is not just a ``rule''---it's an \textbf{operation}. Given inputs $A$ and $A \to B$, it produces output $B$. A proof system is defined by which operations you allow.

When we ask ``why not use truth tables?''---we're really asking: which operations should count as valid proof steps? The answer determines what ``proof'' means.
